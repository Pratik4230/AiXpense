...
import type { Category, ExpenseType } from "@/constants/expense";
...
export function useCreateRecurringPayment() {
  ...
  return useMutation({
    mutationFn: (input: CreateRecurringPaymentInput) =>
      api
        .post<{ data: RecurringPayment }>("/recurring", input)
        .then((r) => r.data),
...
export function useUpdateRecurringPayment() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...input }: UpdateRecurringPaymentInput & { id: string }) =>
      api
        .patch<{ data: RecurringPayment }>(`/recurring/${id}`, input)
        .then((r) => r.data),
    onSuccess: () => qc.invalidateQueries({ queryKey: QUERY_KEY }),
  });
}
...
export function useDeleteRecurringPayment() {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      api.delete<{ success: boolean }>(`/recurring/${id}`).then((r) => r.data),
    onSuccess: () => qc.invalidateQueries({ queryKey: QUERY_KEY }),
  });
}
