...
export const createSearchTransactionsTool = ({
  ...
  return tool({
    ...
    execute: async ({
      ...
      console.log(
        "1. Tool Input:",
        JSON.stringify({ query, filter, includeDateFilter }, null, 2),
      );
      await connectDB();
      ...
      if (query && !filter && !aggregation) {
        try {
          console.log("2. Invoking Specialist Agent (gpt-5.1)...");
          ...
          console.log("3. Specialist Generated Output:");
      ...
      console.log("4. Date Logic:");
      console.log("   User Provided Date?", userProvidedDateFilter);
      console.log(
        "   Applying Default (Current Month)?",
        shouldApplyDefaultDate,
      );
      if (shouldApplyDefaultDate) {
        console.log(
          `   Range (UTC): ${monthStartUTC.toISOString()} -> ${monthEndUTC.toISOString()}`,
        );
      }
      ...
      try {
        if (finalAggregation && finalAggregation.length > 0) {
          ...
          console.log(`6. Results: ${results.length} items found`);
        ...
        console.log(
          "5. Final Find Filter:",
          JSON.stringify(safeFilter, null, 2),
        );
